...
import { signoutUser } from "../../Features/Auth/authSlice"; // ✅ import logout thunk

const AccountDashboard = () => {
  const [activePage, setActivePage] = useState("");
  const [sidebarOpen, setSidebarOpen] = useState(true);
  const dispatch = useDispatch();
  const { loading } = useSelector((state) => state.auth); // track logout loading
  ...
  const handleLogOut = () => {
    dispatch(signoutUser()); // ✅ dispatch the logout thunk
  };
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {[{
                ...
                <div key={index} className="flex items-center gap-4 border p-3 rounded-xl shadow hover:shadow-lg transition bg-white">
  ...
  return (
    <>
      ...
      <div className="md:hidden flex justify-end px-4 mt-4">
        <button onClick={() => setSidebarOpen(!sidebarOpen)} className="p-2 bg-gray-200 rounded-full">
      ...
      <div className="flex flex-col md:flex-row w-[95%] mx-auto mt-4 md:mt-26 border border-gray-100 h-screen lg:bg-gray-100">
        {/* Sidebar */}
        <div className={`fixed md:relative top-0 left-0 h-auto w-full lg:w-84 max-wfull bg-white border-r p-5 transform ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} transition-transform duration-300 md:translate-x-0`}>
          <div className="flex items-center mt-10 lg:mt-0 bg-amber-50 p-2 rounded-2xl gap-3 mb-6">
            <div className="w-12 h-12 bg-yellow-400 rounded-full flex items-center justify-center text-white font-bold">CJ</div>
            <div>
          ...
          <nav className="space-y-2 overflow-y-auto max-h-[calc(100vh-100px)]">
            {menuItems.map((item) => (
              <button key={item.key} onClick={() => { setActivePage(item.key); setSidebarOpen(false); }}
                className={`flex items-center gap-3 w-full px-3 bg-blue-50 py-2 rounded-lg text-left transition ${activePage === item.key ? "bg-blue-100 text-blue-600 font-semibold" : "hover:bg-gray-100 text-gray-700"}`}>
                {item.icon} {item.label}
              </button>
            ))}

            <button onClick={handleLogOut} className="flex items-center gap-3 w-full px-3 py-2 rounded-lg bg-red-50 text-left text-red-600 hover:bg-red-100 mt-4">
