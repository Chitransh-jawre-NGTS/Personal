  ...
  {wishlistItems.length > 0 ? (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
      {wishlistItems.map((item) => (
        <div
          key={item.id}
          className="bg-white/60 backdrop-blur-lg border border-gray-200 rounded-2xl shadow-md hover:shadow-xl transition overflow-hidden"
        >
          <img
            src={item.image}
            alt={item.title}
          ...
          <div className="p-5 flex flex-col justify-between h-full">
            <div className="flex flex-col gap-2 mb-4">
              ...
              <div className="flex items-center gap-2">
                <span className="text-gray-400 line-through text-sm">₹{getOriginalPrice(item)}</span>
            ...
            <div className="flex flex-col sm:flex-row gap-3 mt-auto">
              <button
                ...
                className="w-full px-4 py-2 bg-pink-600 hover:bg-pink-700 text-white text-sm font-semibold rounded-full transition-all"
              >
                🛒 Move to Cart
              </button>
              <button
                onClick={() => handleRemoveFromWishlist(item.id)}
                className="w-full px-4 py-2 border border-red-500 text-red-600 hover:bg-red-100 rounded-full text-sm font-semibold transition-all"
              >
                ❌ Remove
              </button>
            </div>
    ...
    <div className="text-center mt-20 text-gray-500 text-lg">
      ...
      <Link to="/" className="text-pink-600 hover:underline mt-4 block">Browse products</Link>
    </div>
  )}
</div> */}
